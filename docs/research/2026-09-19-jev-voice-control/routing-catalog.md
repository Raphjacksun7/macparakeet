...
Status: **research-backed proposal, not implemented or shipped**. Date: 2026-09-19. This document specifies proposed behavior for a full MacParakeet feature and its staged delivery. Example commands are design/evaluation cases, not measured usage popularity. No API calls, desktop actions, or tests were performed for this document.
...
Jev supplies semantic decisions. Local code owns candidate discovery, literal spans, arithmetic, authorization, execution, and outcome evidence. Cloud use is opt-in and text-only: local STT produces the command; filtered UI labels and necessary contextual text go to Jev. Native Accessibility is the primary observation/execution adapter. A user-authorized browser extension supplies richer browser identity, tab/frame context and DOM capabilities; production does not require CDP, a debugging profile, or browser restart. Local OCR is a separately permissioned recovery adapter, with lower-assurance target provenance.
...
[Jev Voice Browser](jev-voice-browser.md) supplies speculative questions, spatial clarification and explicit policy; its stale partials, modified literal text and clarification-policy bypass motivate stronger transactions. [macbrow](macbrow.md) supplies tool routing, missing-slot questions and goal checks; its conflation of clarification with approval motivates separate states. [Third Hand](third-hand.md) supplies operation-specific target heads, AX focus checks and repeat suppression; its misleading Done paths motivate terminal authority revocation and evidence-backed outcomes.

The official [fan-out pattern](https://docs.typesafe.ai/patterns/fan-out) supports independent speculative heads; [function calling](https://docs.typesafe.ai/cookbooks/function_calling) supports typed operation/argument selection; [pre-parsed extraction](https://docs.typesafe.ai/cookbooks/pre_parsed_value_extraction_cookbook) supports choosing source values before local normalization. [Confidence guidance](https://docs.typesafe.ai/confidence) distinguishes probability concentration from correctness; [state guidance](https://docs.typesafe.ai/concepts/state) informs explicit identities and relationships. These pages were read from the parent research task's local snapshots in `references/jev-research-sources`; no fresh external request was made for this writing subtask. The local [TypeSafe skill](../../../.agents/skills/typesafe-ai/SKILL.md) provides matching decomposition guidance.
