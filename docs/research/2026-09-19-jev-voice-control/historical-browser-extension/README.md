...
The JavaScript extension, developer installer, and earlier fixture harness remain under `integrations/voice-control-browser/` for historical inspection only. The earlier recording and `browser-implementation-evidence.md` qualify that retired experimental path; they do not establish native browser control.
...
The active direction is [native Accessibility](../native-accessibility-direction.md), with [correction/recovery and observability](../correction-recovery-observability-direction.md). Current native-browser qualification belongs in `scripts/dev/voice-control/`.
