...
| [2026-09-11-issue-895-meeting-split](../docs/plans/2026-09-11-issue-895-meeting-split-plan.md) | Split accidentally combined saved meetings (#895) | **IMPLEMENTED ON MAIN** | P2 | App + CLI split shipped in CLI 4.1.0 (`meetings split preview/create/status/resume/discard`). Historical research/HTML remains reference; remaining work is product UX polish and real-audio qualification, not a missing kernel. |
...
| [2026-06-12-telemetry-allowlist-ci-guard](active/2026-06-12-telemetry-allowlist-ci-guard.md) | Cross-repo telemetry allowlist CI guard | **IMPLEMENTED** | P2 | Script + CI step land on `feat/telemetry-agent-debug`. Enforcement in GitHub Actions still needs the maintainer `WEBSITE_REPO_TOKEN` secret. |
...
| [2026-09-16-telemetry-observability-followthrough](active/2026-09-16-telemetry-observability-followthrough.md) | Dictation e2e timings, cheap activation SQL, pipeline health, LLM formatter labels | **PARTIAL** | P1 | Website allowlist, ingest remap, windowed activation SQL, and snapshot health probe are live on `/api/stats`. App `dictation_insert` + formatter `feature=formatter` merged as [#1059](https://github.com/moona3k/macparakeet/pull/1059) and shipped in the 0.8.4 DMG. Do not deploy retention. Operator: one Cloudflare Worker-exception notification on `macparakeet-telemetry-snapshots`. |
| [2026-09-15-issue-1046-speaker-over-split](active/2026-09-15-issue-1046-speaker-over-split.md) | Auto speaker over-split / transcript clutter (#1046) | **PARTIAL** | P2 | Ship conservative word smoothing. Frozen VoxConverse A/B showed tau-0.25 centroid consolidation changed 0/7 rosters, so consolidation remains deferred; do not fit tau to the gate. |
...
- [Jev-powered Voice Control](active/2026-09-19-jev-voice-control.md) — DEBUG experiment implemented on `feat/jev-voice-control`; Flights-to-results and mic qualification remain.
