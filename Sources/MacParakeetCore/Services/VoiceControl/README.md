...
- Observation is `AXTreeWalk` (pure, over `AXTreeSource`) plus a candidate pass that reads values, settability, selection and fingerprints only for kept nodes. Rules and caps are documented on the type and covered by `AXTreeWalkTests` against fake trees. Off-screen pressables become `isOffscreen` targets: exact-name local commands may press them; legality filtering keeps them out of every Jev request.
...
`VoiceControlTraceStore` also writes a local session log under `AppPaths.voiceControlLogsDir` (`latest.md`, `latest.json`, `events.jsonl`, and `sessions/`). `latest.md` is the wide event for the current turn. `latest.json` includes the instruction, application name, offered control labels, the same summary, and joinable per-step records. Field values, selected text, audio, screenshots, credentials and remote bodies stay out. End clears the panel; it does not delete the log. Retention is the last 20 sessions. Agents can read `/tmp/macparakeet-voice-control/latest.md`, which points at the same snapshot. Nothing is uploaded.
